...
struct Camera {
    ...
    __device__ __inline__ void getRay(int x, int y, int width, int height, float3* rayOrigin, float3* rayDirection) const {
...
*/
 ...
 __device__ __inline__ void getRay2(int x, int y, int width, int height, Camera camera,
        float3* rayOrigin, float3* rayDirection)
{
        ...
        // Calculate the direction of the ray in world space
        float3 forward = make_float3(camera.target.x - camera.position.x, camera.target.y - camera.position.y, camera.target.z - camera.position.z);
        forward = normalize(forward);
...
__host__ __device__ __inline__ std::optional<Vec3> rayPlaneIntersect(const Ray& ray, const Vec3& planePoint, const Vec3& planeNormal) {
...
__host__ __device__ __inline__ std::optional<Vec3> rayRectangleIntersect(const Ray& ray, const Rectangle& rect) {
...
void buildBVHWithTriangleVersion1(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNode>& nodes)
{
    ...
    for (int i = numTriangles - 2; i >= 0; --i) {
        ...
        //if (i < 24) printf("Internal %d: min(%f,%f,%f) max(%f,%f,%f)\n", i, node.min.x, node.min.y, node.min.z, node.max.x, node.max.y, node.max.z);
...
__global__ void initializeLeaves3(F3Triangle* triangles, BVHNode* nodes, int numTriangles) {
    ...
    if (i < numTriangles) {
        ...
        calculateBoundingBox(triangles[i], node.min, node.max);
        //assert(node.min.x <= node.max.x && node.min.y <= node.max.y && node.min.z <= node.max.z);
        node.triangleIndex = i;
        node.leftChild = node.rightChild = -1;
...
__global__ void buildInternalNodes(BVHNode* nodes, int numTriangles, int level) {
    ...
    if (idx < stride && idx + offset < numTriangles - 1)
    {
        ...
        //assert(node.min.x <= node.max.x && node.min.y <= node.max.y && node.min.z <= node.max.z);
        //if (nodeIdx < 24) printf("Internal %d: min(%f,%f,%f) max(%f,%f,%f)\n", nodeIdx, node.min.x, node.min.y, node.min.z, node.max.x, node.max.y, node.max.z);
    }
...
__global__ void calculateMortonCodes(F3Triangle* triangles, Morton* mortonCodes, int numTriangles, float3 sceneMin, float3 sceneExtent) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numTriangles) {
        float3 centroid = (triangles[idx].v0 + triangles[idx].v1 + triangles[idx].v2) / 3.0f;
        float3 normalized = (centroid - sceneMin) / sceneExtent;
        uint32_t x = __float_as_uint(normalized.x) & 0x3FF;
        uint32_t y = __float_as_uint(normalized.y) & 0x3FF;
        uint32_t z = __float_as_uint(normalized.z) & 0x3FF;
        mortonCodes[idx].code = (x << 20) | (y << 10) | z;
...
__global__ void buildBVH(Morton* sortedMortonCodes, BVHNode* nodes, F3Triangle* triangles, int numTriangles) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numTriangles - 1) return;

    int commonPrefix = __clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx+1].code);
    int direction = (commonPrefix - __clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx-1].code)) < 0 ? -1 : 1;
    
    int minPrefix = __clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx + direction].code);
    int maxPrefix = __clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx - direction].code);

    int d = 2;
    while (__clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx + d * direction].code) > minPrefix)
        d *= 2;

    int delta = d / 2;
    while (delta > 0) {
        if (__clz(sortedMortonCodes[idx].code ^ sortedMortonCodes[idx + (d - delta) * direction].code) > minPrefix)
            d -= delta;
        delta /= 2;
    }
    ...
    if (leftChild < numTriangles) {
        ...
        nodes[leftChild].min = minFloat3(nodes[leftChild].min, triangles[triIdx].v1);
        nodes[leftChild].min = minFloat3(nodes[leftChild].min, triangles[triIdx].v2);
        nodes[leftChild].max = maxFloat3(nodes[leftChild].max, triangles[triIdx].v1);
        nodes[leftChild].max = maxFloat3(nodes[leftChild].max, triangles[triIdx].v2);
        nodes[leftChild].triangleIndex = triIdx;
    }
    ...
    if (rightChild < numTriangles) {
        ...
        nodes[rightChild].min = minFloat3(nodes[rightChild].min, triangles[triIdx].v1);
        nodes[rightChild].min = minFloat3(nodes[rightChild].min, triangles[triIdx].v2);
        nodes[rightChild].max = maxFloat3(nodes[rightChild].max, triangles[triIdx].v1);
        nodes[rightChild].max = maxFloat3(nodes[rightChild].max, triangles[triIdx].v2);
        nodes[rightChild].triangleIndex = triIdx;
    }
...
void buildBVHWithTriangleVersion4(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNode>& nodes) {
    ...
    int blockSize = 256;
    int numBlocks = (numTriangles + blockSize - 1) / blockSize;
    hipLaunchKernelGGL(calculateMortonCodes, dim3(numBlocks), dim3(blockSize), 0, 0, d_triangles, d_mortonCodes, numTriangles, sceneMin, sceneMax - sceneMin);

    // Trier les codes de Morton
    thrust::sort(thrust::device, mortonCodes.begin(), mortonCodes.end(),
                 [] __device__  (const Morton& a, const Morton& b) { return a.code < b.code; });

    // Construire le BVH
    BVHNode* d_nodes = thrust::raw_pointer_cast(nodes.data());
    hipLaunchKernelGGL(buildBVH, dim3(numBlocks), dim3(blockSize), 0, 0, d_mortonCodes, d_nodes, d_triangles, numTriangles);
}
*/
...
__global__ void calculateMortonCodes(const F3Triangle* triangles, Morton* mortonCodes, int numTriangles, float3 sceneMin, float3 sceneExtent) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numTriangles) {
        float3 centroid = (triangles[idx].v0 + triangles[idx].v1 + triangles[idx].v2) / 3.0f;
        float3 normalized = (centroid - sceneMin) / sceneExtent;
...
__global__ void buildBVHLeaves(const Morton* sortedMortonCodes, BVHNode* nodes, const F3Triangle* triangles, int numTriangles) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numTriangles) {
        int leafIdx = numTriangles - 1 + idx;
        int triIdx = sortedMortonCodes[idx].index;
        
        nodes[leafIdx].min = minFloat3(triangles[triIdx].v0, minFloat3(triangles[triIdx].v1, triangles[triIdx].v2));
        nodes[leafIdx].max = maxFloat3(triangles[triIdx].v0, maxFloat3(triangles[triIdx].v1, triangles[triIdx].v2));
...
void buildBVHWithTriangleVersion4(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNode>& nodes) {
    ...
    calculateMortonCodes<<<numBlocks, blockSize>>>(d_triangles, d_mortonCodes, numTriangles, sceneMin, sceneExtent);
    hipDeviceSynchronize();
    // Sort Morton codes
    thrust::sort(thrust::device, mortonCodes.begin(), mortonCodes.end(),
                 [] __device__  (const Morton& a, const Morton& b) { return a.code < b.code; });
    ...
    buildBVHLeaves<<< numBlocks, blockSize>>>(d_mortonCodes, d_nodes, d_triangles, numTriangles);
    hipDeviceSynchronize();
...
__device__ __inline__ bool rayTriangleIntersect(const F3Ray& ray, const F3Triangle& triangle, float& t, float3& intersectionPoint) {
...
__device__ __inline__ __forceinline__ bool rayTriangleIntersectVersion2(const F3Ray& ray, const F3Triangle& triangle, float& t, float3& intersectionPoint) {
 ...
 
{
    ...
    if (hitId[idx]!=-1)
    {
        ...
        if (modeColor==1) {
            ...
            R=1.0-d; B=d; G=sqrt(1.0-B*B-R*R);
            int ypx=(y * width + x) * 3;
...
__device__ __inline__ inline float3 operator-(const float3& a, const float3& b) {
...
)
{
    ...
    while (stackPtr > 0) {
        ...
        if (node.triangleIndex != -1) {
            float t;
            float3 intersectionPointT;
            //if (rayTriangleIntersectVers2(ray, triangles[node.triangleIndex], t, intersectionPointT))
...
__device__ __inline__ __forceinline__
...
__global__ void rayTracingImgKernelVersion3(
    ...
    const F3Triangle*  __restrict__ triangles,
...
__device__ __inline__ Vec3 computeColor(const Vec3& hitPoint, const Vec3& normal,
                              const Vec3& viewDir, const Material& material,
                              const Vec3& lightPos, const Vec3& lightColor) {
    ...
    float spec = powf(fmaxf(reflectDir.dot(viewDir), 0.0f), material.roughness * 128); // Ajustement basé sur la rugosité
...
__device__ __inline__ bool intersectTriangleVersion2(const F3Ray& ray, const F3Triangle& triangle, float& t, float3& intersectionPoint) {
        float3 edge1 = triangle.v1 - triangle.v0;
        float3 edge2 = triangle.v2 - triangle.v0;
        float3 h = cross(ray.direction, edge2);
        float a = dot(edge1, h);
...
__device__ __inline__ __host__ inline float getComponent(const float3& vec, int axis) {
    switch(axis) {
        case 0: return vec.x;
        case 1: return vec.y;
        case 2: return vec.z;
...
void buildPicturRayTracingPPM_AABB(
    thrust::device_vector<F3Triangle>& triangles,
    thrust::device_vector<BVHNodeAABB>& nodes,
    Camera camera,
    int width,
...
__device__ __inline__ inline float3 elementwise_min(const float3& a, const float3& b) {
	return make_float3(fminf(a.x, b.x), fminf(a.y, b.y), fminf(a.z, b.z));
}

__device__ __inline__ inline float3 elementwise_max(const float3& a, const float3& b) {
	return make_float3(fmaxf(a.x, b.x), fmaxf(a.y, b.y), fmaxf(a.z, b.z));
}

__device__ __inline__ void updateNodeBounds(BVHNodeSAH& node, const F3Triangle& triangle) {
...
__device__ __inline__ void updateBinBounds(SAHBinInfo& bin, const F3Triangle& triangle) {
...
/*
__global__ void initializeLeavesSAH(F3Triangle* triangles, BVHNodeSAH* nodes, int numTriangles) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx < numTriangles) {
...
__global__ void initializeLeavesSAH(F3Triangle* triangles, BVHNodeSAH* nodes, int numTriangles) {
	...
	if (idx < numTriangles) {
		...
		// Calcul des bornes minimales et maximales directement à partir des sommets du triangle
		node.bounds_min = elementwise_min(elementwise_min(tri.v0, tri.v1), tri.v2);
		node.bounds_max = elementwise_max(elementwise_max(tri.v0, tri.v1), tri.v2);
...
__global__ void buildInternalNodesSAHOld(F3Triangle* triangles, BVHNodeSAH* nodes, int numTriangles) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numTriangles - 1) return;
	...
	for (int i = 0; i < numTriangles; ++i) {
		float3 centroid = (triangles[i].v0 + triangles[i].v1 + triangles[i].v2) * (1.0f / 3.0f);
	...
	for (int axis = 0; axis < 3; ++axis)
	{
		...
		float axisMin = (axis == 0) ? global_centroid_min.x : ((axis == 1) ? global_centroid_min.y : global_centroid_min.z);
		float axisMax = (axis == 0) ? global_centroid_max.x : ((axis == 1) ? global_centroid_max.y : global_centroid_max.z);
		...
		for (int i = 0; i < numTriangles; ++i) {
			...
			float axisValue = (axis == 0) ? centroid.x : ((axis == 1) ? centroid.y : centroid.z);

			int binIdx = min(numBins - 1, int((axisValue - axisMin) / (axisMax - axisMin) * numBins));
		...
		SAHBinInfo rightAccum = { make_float3(FLT_MAX, FLT_MAX, FLT_MAX), make_float3(-FLT_MAX, -FLT_MAX, -FLT_MAX), 0 };

		for (int i = 1; i < numBins; ++i)
		{
			rightAccum.bounds_min = elementwise_min(rightAccum.bounds_min, bins[i].bounds_min);
			rightAccum.bounds_max = elementwise_max(rightAccum.bounds_max, bins[i].bounds_max);
			rightAccum.count += bins[i].count;
		}

		for (int split = 1; split < numBins; ++split) {
			float cost = (calculateBinCost(leftAccum) * leftAccum.count + calculateBinCost(rightAccum) * rightAccum.count) / numTriangles;
			...
			leftAccum.bounds_min = elementwise_min(leftAccum.bounds_min, bins[split].bounds_min);
			leftAccum.bounds_max = elementwise_max(leftAccum.bounds_max, bins[split].bounds_max);
			leftAccum.count += bins[split].count;

			rightAccum.bounds_min = elementwise_max(rightAccum.bounds_min, bins[split].bounds_min);
			rightAccum.bounds_max = elementwise_min(rightAccum.bounds_max, bins[split].bounds_max);
	...
	if (bestAxis != -1) {
		...
		if (bestCost == 0) {
			splitPos = (getComponent(global_centroid_min, bestAxis) + getComponent(global_centroid_max, bestAxis)) * 0.5f;
		}
		else {
			splitPos = getComponent(global_centroid_min, bestAxis) +
				(getComponent(global_centroid_max, bestAxis) - getComponent(global_centroid_min, bestAxis)) * (float)bestSplit / numBins;
		}
...
__global__ void buildInternalNodesSAH(F3Triangle* triangles, BVHNodeSAH* nodes, int numTriangles) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numTriangles - 1) return;
	...
	if (bestAxis != -1) {
		...
		if (bestCost == 0) {
			...
			splitPos =
				...
				(getComponent(global_centroid_max, bestAxis) - getComponent(global_centroid_min, bestAxis)) * static_cast<float>(bestSplit) / numBins;
		}
...
void buildBVHWithTriangleVersion3SAH(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNodeSAH>& nodes)
{
	int numTriangles = triangles.size();
	nodes.resize(2 * numTriangles - 1);
	...
	initializeLeavesSAH <<<numBlocks, blockSize >>> (thrust::raw_pointer_cast(triangles.data()),
		thrust::raw_pointer_cast(nodes.data()),
		numTriangles);
	...
	buildInternalNodesSAH <<<numBlocks, blockSize >>> (thrust::raw_pointer_cast(triangles.data()),
		thrust::raw_pointer_cast(nodes.data()),
		numTriangles);
	hipDeviceSynchronize();
	...
	checkBVHConsistency <<<(numNodes + 255) / 256, 256 >>> (thrust::raw_pointer_cast(nodes.data()), numNodes);
	hipDeviceSynchronize();
}
...
__global__ void rayTracingImgKernelVersion3SAH(
	...
	while (stackPtr > 0) {
		...
		if (nodeIdx > 0) printf("idx [%i] nodeIdx[%i] stask [%i] %i %i\n", idx, nodeIdx, stackPtr, node.left, node.right);
		...
		if (dirIsNeg[2]) SWAP(float, tzmin, tzmax);
		...
		if (stackPtr < MAX_STACK_SIZE - 2) { // Ensure space for both children
			if (node.right != -1) stack[stackPtr++] = node.right; // Push right child first for backtracking
			if (node.left != -1) stack[stackPtr++] = node.left;   // Push left child next
		}
	}
...
void buildPicturRayTracingPPM3_SAH(
	thrust::device_vector<F3Triangle>& triangles,
...
void writeBVHNodesSAH1(const thrust::device_vector<BVHNodeSAH>& nodes) {
	...
	for (size_t i = 0; i < hostNodes.size(); ++i) {
		...
		std::cout << "  Bounds:\n";
		std::cout << "    Min: ";
		std::cout << "(" << node.bounds_min.x << ", " << node.bounds_min.y << ", " << node.bounds_min.z << ")" << "\n";
		std::cout << "    Max: ";
		std::cout << "(" << node.bounds_max.x << ", " << node.bounds_max.y << ", " << node.bounds_max.z << ")" << "\n";
		std::cout << std::endl;
	}
...
struct HitInfo {
    ...
    __device__ __inline__ HitInfo() : t(FLT_MAX), normal(make_float3(0.0f,0.0f,0.0f)), triangleIndex(-1) {}
};
...
__device__ __inline__ float evaluateSAH(
    const AABB_SAH2& nodeBounds,
    const AABB_SAH2& leftBounds,
    const AABB_SAH2& rightBounds,
    ...
    float intersectionCost)
{
    ...
    return traversalCost + intersectionCost * (pLeft * leftCount + pRight * rightCount);
}




__global__ void buildBVHKernelSAH2(F3Triangle* triangles, BVHNodeSAH2* nodes, int numTriangles) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // Si le nœud est une feuille ou ne contient qu'un seul triangle, on initialise ses bornes 
    ...
    for (int i = node.left; i < node.left + node.count; ++i) {
        ...
        printf("node.left=%i <%f,%f,%f> <%f,%f,%f>\ ",i,nodeBounds.min.x,nodeBounds.min.y,nodeBounds.min.z,nodeBounds.max.x,nodeBounds.max.y,nodeBounds.max.z);

        int bucketIndex = numBuckets * maxComponent((centroid - nodeBounds.min) / (nodeBounds.max - nodeBounds.min));
        ...
        buckets[bucketIndex].bounds.expand(tri.v2);
         ...
         printf("bucket <%f,%f,%f> <%f,%f,%f>\n",buckets[bucketIndex].bounds.min.x,buckets[bucketIndex].bounds.min.y,buckets[bucketIndex].bounds.min.z,
         buckets[bucketIndex].bounds.max.x,buckets[bucketIndex].bounds.max.y,buckets[bucketIndex].bounds.max.z);
    ...
    for (int i = 1; i < numBuckets; ++i) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            leftCount += buckets[j].count;
             ...
             printf("node left <%f,%f,%f> <%f,%f,%f> \n",leftBounds.min.x,leftBounds.min.y,leftBounds.min.z,leftBounds.max.x,leftBounds.max.y,leftBounds.max.z);
        }
        ...
        float cost = evaluateSAH(nodeBounds, leftBounds, rightBounds, leftCount, rightCount, traversalCost, intersectionCost);
    ...
    if (minCostSplitBucket != -1) {
        ...
        nodes[rightChildIndex] = BVHNodeSAH2(splitIndex, node.count - (splitIndex - node.left));
...
void buildBVHWithTriangleVersion3SAH2(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNodeSAH2>& nodes) {
    int numTriangles = triangles.size();
    nodes.resize(2 * numTriangles - 1);
...
/*
__global__ void buildBVHKernelSAH2(F3Triangle* triangles, BVHNodeSAH2* nodes, int numTriangles, int* nodesToProcess, int numNodesToProcess) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // Si le nœud est une feuille ou ne contient qu'un seul triangle, ne rien faire
    if (node.isLeaf || node.count <= 1) return;
    ...
    for (int i = node.left; i < node.left + node.count; ++i) {
        ...
        int bucketIndex = min(numBuckets - 1, int(numBuckets * maxComponent((centroid - nodeBounds.min) / (nodeBounds.max - nodeBounds.min))));
...
void buildBVHWithTriangleVersion3SAH2(thrust::device_vector<F3Triangle>& triangles, thrust::device_vector<BVHNodeSAH2>& nodes) {
    ...
    int h_numNodesToProcess = numTriangles; // Host variable to store the number of nodes to process
    hipMemcpy(d_numNodesToProcess, &h_numNodesToProcess, sizeof(int), hipMemcpyHostToDevice);
    ...
    while (true) {
        ...
        int temp;
        hipMemcpy(&temp, d_numNodesToProcess, sizeof(int), hipMemcpyDeviceToHost);
        if (temp == 0) break;
...
__device__ __inline__ bool intersectAABB_SAH2(const F3Ray& ray, const AABB_SAH2& aabb) {
	float3 invDir = make_float3(1.0f / ray.direction.x, 1.0f / ray.direction.y, 1.0f / ray.direction.z);
	float3 t0 = make_float3((aabb.min.x - ray.origin.x) * invDir.x,
		(aabb.min.y - ray.origin.y) * invDir.y,
		(aabb.min.z - ray.origin.z) * invDir.z);
...
__device__ __inline__ bool intersectTriangle(const F3Ray& ray, const F3Triangle& triangle, HitInfo& hitInfo) {
...
__device__ __inline__ bool traverseBVH(const F3Ray& ray, const BVHNodeSAH2* nodes, const F3Triangle* triangles, HitInfo& hitInfo) {
	int stack[64];
	int stackPtr = 0;
	...
	while (stackPtr > 0) {
		...
		if (!intersectAABB_SAH2(ray, node.bounds)) continue;
...
__global__ void rayTracingKernelSAH2(
	unsigned char* image,
	const int width,
	const int height,
	const Camera camera,
