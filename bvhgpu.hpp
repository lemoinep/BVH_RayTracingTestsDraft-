...
struct AABB_SAH2 {
    ...
    __host__ __device__ __inline__ void expand(const float3& point) {
        min = make_float3(fminf(min.x, point.x), fminf(min.y, point.y), fminf(min.z, point.z));
        max = make_float3(fmaxf(max.x, point.x), fmaxf(max.y, point.y), fmaxf(max.z, point.z));
    }
    ...
    __host__ __device__ __inline__ AABB_SAH2(): min(make_float3(FLT_MAX, FLT_MAX, FLT_MAX)),max(make_float3(-FLT_MAX, -FLT_MAX, -FLT_MAX)) {}
...
struct BVHNodeSAH2 {
    ...
    bool isLeaf;
    AABB_SAH2 bounds;
    __host__ __device__ __inline__ BVHNodeSAH2() : left(0), count(0), isLeaf(true) {}
    //__host__ __device__ __inline__ BVHNodeSAH2(int _left, int _count) : left(_left), count(_count), isLeaf(_count == 1) {}
    //__host__ __device__ __inline__ BVHNodeSAH2(int _left = 0, int _count = 0) : left(_left), count(_count), isLeaf(_count <= 1) {}

    __host__ __device__ __inline__ BVHNodeSAH2(int _left, int _count)  : left(_left), count(_count), isLeaf(_count == 1), bounds() {} 
};
...
struct MaterialPlus {
    ...
    __host__ __device__ __inline__ MaterialPlus(Vec3 a, float r, float m, float refl = 0.0f, 
                                  float trans = 0.0f, float refrIndex = 1.0f) 
