...
bool loadOBJTriangle(const std::string &filename,
                     std::vector<F3Triangle> &triangles, const int &id) {
...
void testRayTracingPicture005(const std::string &filename,
  ...
  t_laps =
      std::chrono::duration_cast<std::chrono::microseconds>(t_end_1 - t_begin_1)
          .count();
  std::cout << "[INFO]: Elapsed microseconds inside Ray : " << t_laps
            << " us\n";

  t_laps =
      std::chrono::duration_cast<std::chrono::microseconds>(t_end_1 - t_begin_0)
          .count();
  std::cout << "[INFO]: Elapsed microseconds inside Application: " << t_laps
            << " us\n";
...
void testRayTracingPicture006(const std::string &filename,
                              const std::string &filenamePPM, int numVersion) {
  ...
  if (numVersion == 5)
    buildBVHWithTriangleVersion5(deviceTriangles, deviceNodes);
  thrust::device_vector<BVHNodeAABB> deviceNodes_AABB;
  if (numVersion == 6)
    buildBVH_AABB(deviceTriangles, deviceNodes_AABB);

  thrust::device_vector<BVHNodeSAH> deviceNodes_SAH;
  if (numVersion == 7)
    buildBVHWithTriangleVersion3SAH(deviceTriangles, deviceNodes_SAH); // ERROR

  thrust::device_vector<BVHNodeSAH2> deviceNodes_SAH2;
  if (numVersion == 8)
    buildBVHWithTriangleVersion3SAH2(deviceTriangles, deviceNodes_SAH2); // ERROR
  ...
  if (numVersion == 8)
    writeBVHNodesSAH2(deviceNodes_SAH2);

  bool isSave = true; // isSave=false;
  t_begin_1 = std::chrono::steady_clock::now();
  if (numVersion < 6)
    buildPicturRayTracingPPM(deviceTriangles, deviceNodes, camera, width,
                             height, filenamePPM, isSave);
  if (numVersion == 6)
    buildPicturRayTracingPPM_AABB(deviceTriangles, deviceNodes_AABB, camera,
                                  width, height, filenamePPM, isSave);

  // SAH
  if (numVersion == 7)
    buildPicturRayTracingPPM3_SAH(deviceTriangles, deviceNodes_SAH, camera,
                                  width, height, filenamePPM, isSave); // ERROR
  if (numVersion == 8)
    buildPicturRayTracingPPM3_SAH2(deviceTriangles, deviceNodes_SAH2, camera,
                                   width, height, filenamePPM, isSave); // ERROR
  t_end_1 = std::chrono::steady_clock::now();
...
void buildExplorationLight() {
  ...
  for (double theta = thetaStart; theta <= thetaEnd; theta += thetaStep) {
    for (double alpha = alphaStart; alpha <= alphaEnd; alpha += alphaStep) {
      ...
      std::cout << "Origine: <" << ray_origin[0] << ", " << ray_origin[1]
                << ", " << ray_origin[2] << ">"
                << " ";
      std::cout << "Direction: <" << ray_direction[0] << ", "
